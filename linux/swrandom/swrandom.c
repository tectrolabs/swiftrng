...
static int usb_probe(struct usb_interface *interface, const struct usb_device_id *id) {
	...
	if(mutex_lock_killable(&dataOpLock) != SUCCESS) {
		pr_err( "Could not lock the mutex\n");
		return -EPERM;
	}

	if (isEntropySrcRdy || usbData != NULL) {
		pr_info( "A SwiftRNG device already connected\n");
		mutex_unlock(&dataOpLock);
		return -EPERM;
	}

	if (isShutDown) {
		pr_info( "Cannot register USB device (%04X:%04X) while module is being removed from the kernel\n", id->idVendor, id->idProduct);
		mutex_unlock(&dataOpLock);
		return -EPERM;
	}
	...
	if (retval == SUCCESS && !(usbData->bulk_in_endpointAddr && usbData->bulk_out_endpointAddr)) {
		pr_info( "Could not find both bulk-in and bulk-out endpoints");
		retval = -EPERM;
	}
	if (retval != SUCCESS) {
		clean_up_usb();
...
static void usb_disconnect(struct usb_interface *interface) {
	if(mutex_lock_killable(&dataOpLock) != SUCCESS) {
		pr_info( "Could not lock the mutex\n");
	}
...
static ssize_t device_read(struct file *file, char __user *buffer, size_t length, loff_t * offset)
{
	...
	if (isEntropySrcRdy) {
		...
		if (debugMode) {
			if (total > length) {
				pr_err( "Expected %d bytes to read and actually got %d \n", (int)length, (int)total);
			}
		}
	}
...
/**
 * Print a notification when SwiftRNG device is connected
 */
static void print_welcome_message(void) {
	pr_info( "-----------------------------------------\n");
	pr_info( "-- SwiftRNG device connected and ready --\n");
	pr_info( "-----------------------------------------\n");
}
...
static bool is_post_processing_enabled(void) {
	...
	if ((ctrlData->deviceVersionLong >= 12 && disablePostProcessing)
			|| (ctrlData->deviceVersionLong >= 20 && !strcmp("", postProcessingMethod))) {
...
static int get_device_version(void) {
	...
	retval = snd_rcv_usb_data(ctrlData->bulk_out_buffer, 1,
			ctrlData->deviceVersion, sizeof(ctrlData->deviceVersion) - 1, 10);
...
static int rcv_rnd_bytes(void) {
	...
	if (ctrlData->isPowerProfileSet == false) {
		...
		if (retval == SUCCESS) {
			...
			pr_info( "SwiftRNG device power profile has been set to %d \n", powerProfile);
		} else {
			retval = -EPERM;
		}
	}
	...
	if (ctrlData->isVersionRetrieved == false) {
		...
		if (retval == SUCCESS) {
			...
			pr_info( "SwiftRNG device version: %ld \n", ctrlData->deviceVersionLong);
			if (is_post_processing_enabled() == true) {
				pr_info( "SwiftRNG post processing is currently enabled\n");
				...
				pr_info( "SwiftRNG post processing is currently disabled\n");
			}
		} else {
			retval = -EPERM;
		}
		if (disableStatisticalTests) {
			pr_info( "SwiftRNG statistical tests are currently disabled\n");
		} else {
			pr_info( "SwiftRNG statistical tests are currently enabled\n");
		}
	...
	if (retval == SUCCESS) {
    	...
    	if (retval == SUCCESS) {
    		...
    		if (rct.statusByte != SUCCESS) {
    			pr_err( "Repetition Count Test failure\n");
    			retval = -EPERM;
    		} else if (apt.statusByte != SUCCESS) {
    			pr_err( "Adaptive Proportion Test failure\n");
    			retval = -EPERM;
    		}
    	}
	}
	...
	for (retry = 0; retry < USB_READ_MAX_RETRY_CNT; retry++) {
		...
		if (retval == SUCCESS && actualcCnt == sizeSnd) {
			...
			if (retval == SUCCESS) {
				if (rcv[sizeRcv] != 0) {
					retval = -EFAULT;
					pr_err( "SW RNG internal failure: received device status code %d\n", rcv[sizeRcv]);
				} else {
					break;
				}
			}
			...
			if (debugMode) {
				pr_info( "It was an error during data communication. Cleaning up the receiving queue and continue.\n");
			}
...
static int chip_read_data(char *buff, int length, int opTimeoutSecs) {
	...
	do {
		...
		if (acmCtxt->device_locked == false) {
			...
			retval = usb_bulk_msg(usbData->udev, usb_rcvbulkpipe(usbData->udev, usbData->bulk_in_endpointAddr), ctrlData->bulk_in_buffer, length, &transferred, HZ*20);
		} else {
		...
		if (transferred > USB_BUFFER_SIZE) {
			pr_err( "Received unexpected bytes when processing USB device request\n");
			return -EFAULT;
		}
...
static int init_char_dev(void) {
	...
	if (error < 0) {
		pr_err( "alloc_chrdev_region() call failed with error: %d\n", error);
		return error;
	}
	major = MAJOR(dev);
...
static int create_device(void) {
	...
	if (error)
	{
		pr_err( "cdev_add() call failed with error: %d\n", error);
		return error;
	}
	device = device_create(dev_class, NULL, devno, NULL, DEVICE_NAME);
	if (IS_ERR(device)) {
		error = PTR_ERR(device);
		pr_err( "device_create() failed with error: %d\n", error);
		return error;
	}
...
static int __init init_swrandom(void)
{
	...
	if (bytesPerSample <= 0 || bytesPerSample > TRND_OUT_BUFFSIZE) {
		pr_err( "Bytes per second parameter %d is not valid, it must be between 1 and 16000\n", bytesPerSample);
		return -EINVAL;
	}

	if (powerProfile < 0 || powerProfile > 9)
	{
		pr_err( "Power profile parameter %d is not valid, it must be between 0 and 9\n", (int)powerProfile);
		return -EINVAL;
	}
	...
	if (!strcmp("SHA256", postProcessingMethod) || !strcmp("", postProcessingMethod)) {
		...
		pr_err( "Post processing method %s is not valid\n", postProcessingMethod);
		return -EINVAL;
	}

	sha256_initializeSerialNumber(413145);
	if (sha256_selfTest() != SUCCESS) {
		pr_err( "Post processing logic SHA256 failed the self-test\n");
		return -EPERM;
	}

	if (sha512_selfTest() != SUCCESS) {
		pr_err( "Post processing logic SHA512 failed the self-test\n");
		return -EPERM;
	}

	if (xorshift64_selfTest() != SUCCESS) {
		pr_err( "Post processing logic xorshift64 failed the self-test\n");
		return -EPERM;
	}

	err = init_char_dev();
	if (err != SUCCESS) {
		pr_err( "Could not initialize char device %s\n", DEVICE_NAME);
		return err;
	}
	...
	if (acmCtxt == NULL) {
		pr_err( "Could not allocate bytes for the acm_context structure\n");
		err = -ENOMEM;
	...
	if (buffRndIn == NULL) {
		pr_err( "Could not allocate %d kernel bytes for the random input buffer\n", RND_IN_BUFFSIZE);
		err = -ENOMEM;
	...
	if (buffTRndOut == NULL) {
		pr_err( "Could not allocate %d kernel bytes for the random output buffer\n", TRND_OUT_BUFFSIZE);
		err = -ENOMEM;
	...
	if (ctrlData == NULL) {
		pr_err( "Could not allocate %d kernel bytes for control data\n", TRND_OUT_BUFFSIZE);
		err = -ENOMEM;
		goto ctrl_mem_err;
	}
	...
	if (usb_result < 0) {
		pr_err( "Could not register usb driver, error number %d\n", usb_result);
		err = usb_result;
		goto usb_register_err;
	}

	pr_info( "Char device %s registered successfully with the major number %d, module version: %s\n", DEVICE_NAME, major, DRIVER_VERSION);
...
static int acm_full_read(unsigned char *data, int size, int *bytesTransfered) {
	...
	while (totalBytesReceived < size) {
	    ...
	    if (bytesReceived < 0) {
	    	pr_err("Could not receive data from ACM device");
	    	return -EPERM;
	    }
	    if (bytesReceived == 0) {
	    	return -EPERM;
...
/**
 * A function used for searching ACM devices
 *
 */
static int acm_iterate_dir_callback(struct dir_context *ctx, const char *name,
		int nameLength, loff_t offset, u64 ino, unsigned int dType) {
	struct acm_callback_context *buf =
			container_of(ctx, struct acm_callback_context, ctx);
	return buf->filler(buf->context, name, nameLength, offset, ino, dType);
}

/**
 * A function used when searching for ACM devices
 *
 * @param context
 * @param filler
 * @param path
...
static int acm_readdir(const char* path, acm_readdir_t filler, void* context) {
	int res;
	struct acm_callback_context buf = { .ctx.actor =
			(filldir_t) acm_iterate_dir_callback, .context = context, .filler =
			filler };

	struct file* dir =
			filp_open(path, O_DIRECTORY, S_IRWXU | S_IRWXG | S_IRWXO);
	if (!IS_ERR(dir)) {
 ...
 */
static int acm_filldir_callback(void* data, const char *name, int nameLength,
		loff_t offset, u64 ino, unsigned int dType) {
	if (nameLength >= ACM_DEV_NAME_BY_ID_LENGTH_LIMIT - 1) {
		pr_err("ACM device name too long\n");
		return 0;
	}
	if (acmCtxt->devices_found >= MAX_ACM_DEVICES_TO_PROBE) {
		pr_err("Exceeding max number of ACM devices\n");
		return 0;
	}
...
static bool acm_lock_device(void) {
	...
	if (op_status != 0) {
		pr_info("Could not lock device %s\n", acmCtxt->dev_name);
		return false;
	}
...
static void acm_clean_up(void) {
	...
	if (acmCtxt->device_locked) {
		...
		op_status = locks_lock_inode_wait(acmCtxt->inode, &acmCtxt->fl);
		if (op_status != 0) {
			pr_info("Could not unlock %s\n", acmCtxt->dev_name);
		}
		acmCtxt->device_locked = false;
		if (debugMode) {
			pr_info("locks_lock_inode_wait() returned: %d\n", op_status);
		}
	}
...
static void __exit exit_swrandom(void)
{
	...
	pr_info( "Char device %s unregistered successfully\n", DEVICE_NAME);
}
