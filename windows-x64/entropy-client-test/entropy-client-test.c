...
// Testing thread 
//
unsigned int __stdcall testingThread(void* p) {
...
// Pipe Testing thread 
//
unsigned int __stdcall testingPipeThread(void* p) {
	HANDLE hPipe;
...
int main(int argc, char **argv) {
	...
	if (argc < 2) {
		...
		strcpy_s(pipeEndPointASCII, PIPENAME_MAX_CHARS, defaultPipeEndpoint);
	} else {
		mbstowcs_s(&numCharConverted, pipeEndPoint, PIPENAME_MAX_CHARS, argv[1], strlen(argv[1]) + 1);
		strcpy_s(pipeEndPointASCII, PIPENAME_MAX_CHARS, argv[1]);
	}

	printf("\n");
	wprintf_s(TEXT("Using named pipe:  %s\n"), pipeEndPoint);
	printf("\n");
